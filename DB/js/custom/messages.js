...
jQuery(document).ready(function () {
    ...
    BT.Sl.Requeset("S_Select_Messages", (["user"]), function (r) {
        if (r.value != true) {
            for (var i = 0; i < r.value.length; i++) {
                var tr = $("<tr class='unread'></tr>").appendTo($("#table_messages tbody")[0]);

                $('<td class="aligncenter"><input type="checkbox" name="" /></td>').appendTo(tr[0]);
                $('<td class="star"> ' + r.value[i].NickName + '</td>').appendTo(tr[0]);
                $('<td>' + r.value[i].sentDate + '</td>').appendTo(tr[0]);
                $('<td>' + r.value[i].content + '</td>').appendTo(tr[0]);
                $('<td class="attachment">' + r.value[i].sEmail + '</td>').appendTo(tr[0]);
                $('<td class="date">' + r.value[i].sname + '</td>').appendTo(tr[0]);
                $('<td class="date">' + r.value[i].sQQ + '</td>').appendTo(tr[0]);
            }
        }
    });
