Namespace.register("BT.Ed"); //事件处理模块
    ...
    var create_editor = function ($textarea, classes, placeholder, toolbar_position, toolbar_buttons, toolbar_submit, label_dropfileclick,
        ...
        var content_insertimage = function (wysiwygeditor) {
            ...
            var insert_image_wysiwyg = function (url, filename) {
                var html = '<img id="wysiwyg-insert-image" src="" alt=""' + (filename ? ' title="' + filename.replace(/"/, '&quot;') + '"' : '') + ' /><br />';
                ...
                if (max_imagesize) {
                    $image.css({ maxWidth: max_imagesize[0] + 'px',
                        ...
                        var image_width = $image.width(), image_height = $image.height();
                        if (image_width > max_imagesize[0] || image_height > max_imagesize[1]) {
                            if ((image_width / image_height) > (max_imagesize[0] / max_imagesize[1])) {
                                ...
                                //image_width = parseInt(image_width / image_height * (max_imagesize[1] + 100));
                                //image_height = max_imagesize[1] + 50;
                            }
            ...
            $fileuploader = $('<a id="pis"></a>').attr('draggable', 'true').css({ position: 'absolute', left: 0, top: 0, width: '100%', height: '100%', opacity: 0, cursor: 'pointer' });

            if ($fileuploader) $('<div/>').addClass('wysiwyg-browse').html(label_dropfileclick).append($fileuploader).appendTo($content);
            var $button = toolbar_button(toolbar_submit);
        ...
        var get_toolbar_handler = function (name, popup_callback) {
            BT.Ed.wysiwygeditor = wysiwygeditor;
            switch (name) {
                case 'insertimage':
                    if (!popup_callback)
                        return null;
                ...
                case 'AllSeat':
                    return function () {
                        wysiwygeditor.AllScreen(wysiwygeditor);
                       // wysiwygeditor.align('right'); // .closePopup().collapseSelection()
                    };
            }
            return null;
        ...
        var fixed_parent = function () {
            ...
            $(wysiwygeditor.getElement()).parent().prev().each(function (index, element) {
    ...
    var a = new plupload.Uploader({//创建实例的构造方法
        ...
        init: {
            ...
            FileUploaded: function (up, file, info) { //文件上传成功的时候触发
                ...
                wysiwygeditor.insertImage(data.pic);
                $(".wysiwyg-popup").hide();
            },
