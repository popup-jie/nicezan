    ...
    */
    ...
    window.wysiwyg = function (option) {
        ...
        if (showPlaceholder || syncTextarea) {
            ...
            var QEDITOR_DISABLE_ATTRIBUTES_ON_PASTE = ["style", "class", "id", "name", "width", "height"];
            //addEvent(node_wysiwyg, 'paste', debounced_changeHandler);
            addEvent(node_wysiwyg, 'paste', function () {
                var txt = $(this);
                ...
                return setTimeout(function () {
                    ...
                    for (var i = 0; i < els.length; i++) {
                        if (els[i].localName == "p") {
                            var _text = $(els[i]).html();
                            $(els[i]).css({ "margin-bottom": "10px", "text-indent": "12px" });
                        }
                        else if (els[i].localName == "img") {
                            $(els[i]).css({ "margin": "10px 0px 10px" });
                        }
                        else if (els[i].localName == "div") {
                            $(els[i]).css({ "margin-bottom": "10px", "text-indent": "12px" });
                        }
                    }
                    //txt.change();
        ...
        return {
            ...
            AllScreen: function (os) {
                $(os.getElement()).parent().parent().parent().css({ "width": $(window).width(), "height": $(window).height(), "position": "absolute", 'left': "0", "top": "62px", "background": "#fff", "z-index": "3" });
                callUpdates();
                return this;
            }
        };
