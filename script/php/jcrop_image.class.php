...
class jcrop_image{
	...
	var $type;
	 ...
	 public function __construct($filepath, $picname,$x,$y,$w,$h,$tw,$th,$imgName,$type) {
	...
	public function crop(){
		...
		imagedestroy($image);
		$file = $filepath.$imgName.time().".".$ext;
		switch($ext){
			case "png":
		...
		setcookie($type,$file,time()+6*7*24*3600);
		//返回function.php 去操作是否要删除原型图和之前的缩略图
